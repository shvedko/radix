...
```go
arena := &Linked{}
// 1. Быстрая запись (12 ГБ/с)// Возвращает ID, который компактно упакован (pid + gid)
id := arena.Write([]byte("high performance data"))
// 2. Чтение через стековый курсор (30 ГБ/с)// Курсор не аллоцирует память в куче
c := arena.Open(id)
buf := make([]byte, 1024)
n := c.Read(buf)
// 3. Мгновенное освобождение (70 ГБ/с)// Место сразу становится доступным для новых записей
arena.Free(id)
```
